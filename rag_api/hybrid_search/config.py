...
ENABLE_HYBRID_SEARCH = get_env_variable("ENABLE_HYBRID_SEARCH", "false").lower() == "true"
DEFAULT_SEARCH_TYPE = get_env_variable("DEFAULT_SEARCH_TYPE", "semantic")
DEFAULT_SEMANTIC_WEIGHT = float(get_env_variable("DEFAULT_SEMANTIC_WEIGHT", "0.7"))

# Advanced Hybrid Search Parameters
HYBRID_FUSION_K = int(get_env_variable("HYBRID_FUSION_K", "60"))
HYBRID_EXPANSION_FACTOR = float(get_env_variable("HYBRID_EXPANSION_FACTOR", "2.0"))
HYBRID_MAX_RETRIES = int(get_env_variable("HYBRID_MAX_RETRIES", "3"))
HYBRID_SEARCH_TIMEOUT = int(get_env_variable("HYBRID_SEARCH_TIMEOUT", "30"))

# Pool Configuration
HYBRID_POOL_MIN_SIZE = int(get_env_variable("HYBRID_POOL_MIN_SIZE", "2"))
HYBRID_POOL_MAX_SIZE = int(get_env_variable("HYBRID_POOL_MAX_SIZE", "10"))

logger.info(f"Hybrid search enabled: {ENABLE_HYBRID_SEARCH}")
if ENABLE_HYBRID_SEARCH:

    logger.info("=" * 50)
    logger.info("HYBRID SEARCH CONFIGURATION")
    logger.info(f"Enabled: {ENABLE_HYBRID_SEARCH}")
    logger.info(f"Default search type: {DEFAULT_SEARCH_TYPE}")
    logger.info(f"Default semantic weight: {DEFAULT_SEMANTIC_WEIGHT} ({DEFAULT_SEMANTIC_WEIGHT*100:.0f}%)")
    logger.info(f"Default BM25 weight: {1-DEFAULT_SEMANTIC_WEIGHT} ({(1-DEFAULT_SEMANTIC_WEIGHT)*100:.0f}%)")
    logger.info(f"Fusion K: {HYBRID_FUSION_K}")
    logger.info(f"Expansion factor: {HYBRID_EXPANSION_FACTOR}")
    logger.info(f"Timeout: {HYBRID_SEARCH_TIMEOUT}s")
    logger.info("=" * 50)
    logger.info(f"Default search type: {DEFAULT_SEARCH_TYPE}")
    logger.info(f"Default semantic weight: {DEFAULT_SEMANTIC_WEIGHT}")
    logger.info(f"Fusion K parameter: {HYBRID_FUSION_K}")
    logger.info(f"Search expansion factor: {HYBRID_EXPANSION_FACTOR}")
