...
const loadTools = async ({
  ...
  const customConstructors = {
    ...
    'nova-canvas': async (toolContextMap) => {
      // Get image files from conversation context for Nova Canvas
      const imageFiles = options.tool_resources?.[EToolResources.image_edit]?.files ?? [];
      let toolContext = '';
      
      if (imageFiles.length > 0) {
        toolContext = `Image files available in conversation context for Nova Canvas:\n${imageFiles.map((file, index) => 
          `  ${index + 1}. ${file.filename} - Use context_image_index: ${index} to reference this image`
        ).join('\n')}\n\nSet use_context_image: true to automatically use uploaded images as reference for generation, variations, or style guidance.`;
        
        toolContextMap['nova-canvas'] = toolContext;
      }

      return new NovaCanvas({
        ...imageGenOptions,
        imageFiles, // Pass image files for context access
      });
    },
  ...
  const imageGenOptions = {
    isAgent: !!agent,
    userId: user, // Add user ID for file paths
    req: options.req,
    fileStrategy,
