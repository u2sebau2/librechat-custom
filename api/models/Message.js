 ...
 */
async function getMessages(filter, select, options = {}) {
  try {
    ...
    if (select) {
      return await Message.find(queryFilter).select(select).sort({ createdAt: 1 }).lean();
    }

    return await Message.find(queryFilter).sort({ createdAt: 1 }).lean();
  } catch (err) {
 ...
 */
async function getMessage({ user, messageId }, options = {}) {
  try {
