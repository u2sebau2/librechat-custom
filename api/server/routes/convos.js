...
const { getAllConversations } = require('~/models/Conversation');
...
// Admin endpoint to get all conversations including soft-deleted ones
router.get('/admin/all', requireJwtAuth, async (req, res) => {
  try {
    const { cursor, limit = 25, isArchived, tags, search, order = 'desc' } = req.query;
    
    // Note: This is an admin endpoint - you may want to add additional permission checks here
    const options = {
      cursor,
    ...
    const dbResponse = await getAllConversations(req.user.id, options);
    ...
    res.status(500).send('Error getting all conversations');
  }
});
