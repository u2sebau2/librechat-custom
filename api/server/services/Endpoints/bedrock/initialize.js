...
const initializeClient = async ({ req, res, endpointOption }) => {
  ...
  // Configuración de citations para documentos PDF
  const bedrockConfig = req.config?.endpoints?.[EModelEndpoint.bedrock] || {};
  const citationsEnabled = bedrockConfig.citationsEnabled !== false; // Default: true
  const maxCitations = bedrockConfig.maxCitations || 10;
  const citationsFormat = bedrockConfig.citationsFormat || 'markdown';
