...
const deleteLocalFile = async (req, file) => {
  ...
  if (file.embedded && process.env.RAG_API_URL) {
    try {
      ...
      // Handle 404 errors gracefully - file may already be deleted from vector store
      if (error.response && error.response.status === 404) {
        logger.debug(`[deleteLocalFile] Vector not found for file ${file.file_id}, continuing with file deletion`);
      } else {
        logger.error(`[deleteLocalFile] Error deleting vectors for file ${file.file_id}:`, error.message);
        // Don't throw error - continue with local file deletion even if vector deletion fails
      }
    }
  }
