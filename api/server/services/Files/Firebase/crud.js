  ...
  if (file.embedded && process.env.RAG_API_URL) {
    try {
      ...
      if (error.response && error.response.status === 404) {
        logger.debug(`[deleteFirebaseFile] Vector not found for file ${file.file_id}, continuing with file deletion`);
      } else {
        logger.error(`[deleteFirebaseFile] Error deleting vectors for file ${file.file_id}:`, error.message);
        // Don't throw error - continue with file deletion even if vector deletion fails
      }
    }
  }
