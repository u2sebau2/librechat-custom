...
const AgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    let response;
    try {
      logger.info('[BEDROCK-CITATIONS-DEBUG] About to send message to client:', {
        endpoint: endpointOption.endpoint,
        provider: endpointOption.provider || client.options?.agent?.provider,
        model: endpointOption.model || client.options?.agent?.model_parameters?.model,
        citationsEnabled: client.citationsEnabled,
        hasAttachments: !!client.options?.attachments?.length
      });
      
      response = await client.sendMessage(text, messageOptions);
      
      logger.info('[BEDROCK-CITATIONS-DEBUG] Message sent successfully');
    } catch (error) {
      logger.error('[BEDROCK-CITATIONS-DEBUG] Error in sendMessage:', {
        error: error.message,
        stack: error.stack,
      ...
      // Si el error es relacionado con content blocks no soportados, intentar una solución alternativa
      if (error.message && error.message.includes('Unsupported content block type')) {
        logger.warn('[BEDROCK-CITATIONS-DEBUG] Detected unsupported content block type error, attempting recovery...');
        
        // Intentar desactivar citations temporalmente y reintentar
        try {
          if (client.citationsEnabled) {
            logger.info('[BEDROCK-CITATIONS-DEBUG] Disabling citations and retrying...');
            client.citationsEnabled = false;
            response = await client.sendMessage(text, messageOptions);
            logger.info('[BEDROCK-CITATIONS-DEBUG] Retry without citations successful');
          } else {
            throw error; // Re-throw si las citations ya estaban deshabilitadas
          }
        } catch (retryError) {
          logger.error('[BEDROCK-CITATIONS-DEBUG] Retry failed:', retryError);
    ...
    if (req.body.files && (client.options?.attachments || client.message_file_map)) {
      userMessage.files = [];
      const messageFiles = new Set(req.body.files.map((file) => file.file_id));
      
      // Agregar archivos de client.options.attachments (flujo normal)
      ...
      // Agregar archivos de message_file_map (PDFs de Bedrock)
      if (client.message_file_map) {
        const messageId = userMessage.messageId;
        const trackedFiles = client.message_file_map[messageId] || [];
        
        for (let file of trackedFiles) {
          // Solo agregar si no está ya en la lista y está en los archivos del request
          const alreadyAdded = userMessage.files.some(f => f.file_id === file.file_id);
          if (!alreadyAdded && messageFiles.has(file.file_id)) {
            userMessage.files.push({ ...file });
            console.log(`[BEDROCK-DEBUG] Added file from message_file_map: ${file.filename}`);
          }
        }
      }
