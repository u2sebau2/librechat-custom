...
export default function useContentHandler({ setMessages, getMessages }: TUseContentHandler) {
  ...
  return useCallback(
    ({ data, submission }: TContentHandler) => {
      ...
      // Handle different content types including citations
      let part: ContentPart;
      
      if (type === ContentTypes.CITATION) {
        // For citations, use the citation data directly
        part = data[ContentTypes.CITATION];
        console.log('[BEDROCK-CITATIONS-V4] Processing citation in content handler:', part);
      } else if (type === ContentTypes.TEXT) {
        const textPart = data[ContentTypes.TEXT];
        part = textPart != null && typeof textPart === 'string' ? { value: textPart } : data[type];
      } else {
        part = data[type];
      }
