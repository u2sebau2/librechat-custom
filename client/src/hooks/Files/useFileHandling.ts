...
type UseFileHandling = {
  ...
  existingFiles?: [string, ExtendedFile][]; // Files already associated with agent
};
...
const useFileHandling = (params?: UseFileHandling) => {
  ...
  const { setFilesLoading, conversation } = useChatContext();
  const setEphemeralAgent = useSetRecoilState(
    ephemeralAgentByConvoId(conversation?.conversationId ?? Constants.NEW_CONVO),
  );
  const setError = (error: string) => setErrors((prevErrors) => [...prevErrors, error]);
  
  // Use provided fileSetter and its state, or fallback to chat context
  const { files: chatFiles, setFiles: setChatFiles } = useChatContext();
  const actualFiles = params?.fileSetter ? new Map() : chatFiles;
  const actualSetter = params?.fileSetter ?? setChatFiles;
  
  const { addFile, replaceFile, updateFileById, deleteFileById } = useUpdateFiles(actualSetter);
  ...
  const startUpload = async (extendedFile: ExtendedFile) => {
    ...
    if (params?.additionalMetadata) {
      for (const [key, value = ''] of Object.entries(metadata)) {
        // Skip tool_resource here - it will be handled explicitly later to avoid duplication
        if (value && key !== 'tool_resource') {
  ...
  const handleFiles = async (_files: FileList | File[], _toolResource?: string) => {
    ...
    // Create combined files for validation that includes existing files of this type  
    const combined = new Map(actualFiles); // Start with current context files
    
    // Add existing files from props (already filtered by component type)
    if (params?.existingFiles) {
      params.existingFiles.forEach(([key, file]) => {
        if (!combined.has(key)) { // Don't override session files
          // Add tool_resource from current context to existing files
          const fileWithToolResource = {
            ...file,
            tool_resource: (params?.additionalMetadata?.tool_resource as EToolResources) || file.tool_resource,
          };
          combined.set(key, fileWithToolResource);
        }
      });
    }
    ...
    try {
      filesAreValid = validateFiles({
        files: combined, // Use combined files for proper validation
        fileList,
        setError,
        endpointFileConfig:
        ...
        toolResource: _toolResource ?? params?.additionalMetadata?.tool_resource,
    ...
    for (const originalFile of fileList) {
      ...
      try {
        ...
        const resolvedToolResource = _toolResource ?? params?.additionalMetadata?.tool_resource;
        ...
        if (finalProcessedFile !== originalFile) {
          ...
          const tool_resource = initialExtendedFile.tool_resource;
          
          // Check capabilities BEFORE adding the file
          if (isAgentsEndpoint(endpoint) && !isImage && tool_resource == null) {
            ...
            deleteFileById(file_id); // Remove the file from the list
    ...
    // Ensure loading state is cleared after processing all files
    setFilesLoading(false);
  };
