 ...
 *
 * First, checks for simple defaultModel config, then admin defined default, then last selected spec, followed by first spec
 */
export function getDefaultModelSpec(startupConfig?: t.TStartupConfig) {
  // 🆕 NUEVA FUNCIONALIDAD: Priorizar defaultModel del YAML
  if (startupConfig?.defaultModel?.endpoint && startupConfig?.defaultModel?.model) {
    const { endpoint, model } = startupConfig.defaultModel;
    
    // Crear un modelSpec sintético basado en defaultModel
    const syntheticSpec = {
      name: `${endpoint}_default`,
    ...
    console.log('[getDefaultModelSpec] Using defaultModel from config:', syntheticSpec);
