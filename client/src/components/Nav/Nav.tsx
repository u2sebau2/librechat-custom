...
const AdminConversationsButton = lazy(() => import('./AdminConversationsButton'));
...
const Nav = memo(
  ({
    ...
    const headerButtons = useMemo(
      () => (
        <>
          ...
          <Suspense fallback={null}>
            <AdminConversationsButton isSmallScreen={isSmallScreen} toggleNav={toggleNavVisible} />
          </Suspense>
    ...
    // Render for resizable nav (desktop only)
    if (isResizable && !isSmallScreen) {
      return (
        <div data-testid="nav" className="h-full w-full bg-surface-primary-alt hide-scrollbar">
          <div className="flex h-full flex-col hide-scrollbar">
            <nav
              id="chat-history-nav"
              aria-label={localize('com_ui_chat_history')}
              className="flex h-full flex-col px-2 pb-3.5 md:px-3 hide-scrollbar"
            >
              <div className="flex flex-1 flex-col hide-scrollbar" ref={outerContainerRef}>
                <MemoNewChat
    ...
    // Original render for non-resizable nav (mobile and fallback)
    return (
      <>
        <div
          data-testid="nav"
        ...
        >
          <div className="h-full w-[320px] md:w-[280px]">
