...
const SidePanel = ({
  ...
  getCollapsedSizePercentage?: () => number;
}) => {
  ...
  const toggleNavVisible = useCallback(() => {
    ...
    setIsCollapsed((prev: boolean) => {
      if (prev) {
        // Expandir: usar configuración normal
        setMinSize(defaultMinSize);
        setCollapsedSize(navCollapsedSize);
        setFullCollapse(false);
        localStorage.setItem('fullPanelCollapse', 'false');
      } else {
        // Colapsar: usar función pasada como prop o calcular
        const collapsedPercentage = getCollapsedSizePercentage ? 
          getCollapsedSizePercentage() : 
          Math.max(1, Math.min(8, (50 / window.innerWidth) * 100));
        
        setCollapsedSize(collapsedPercentage);
        
        console.log('[SidePanel] Collapsing to fixed 50px width:', {
          targetPx: 50,
          viewportWidth: window.innerWidth,
          calculatedPercentage: collapsedPercentage,
          equivalentPx: (window.innerWidth * collapsedPercentage) / 100,
          usingPropsFunction: !!getCollapsedSizePercentage
        });
      }
      return !prev;
    });
    ...
    <>
      ...
      <ResizablePanel
        ...
        style={{
          ...
          // 🆕 Forzar ancho exacto cuando está colapsado
          ...(isCollapsed && !fullCollapse ? { width: '50px', minWidth: '50px', maxWidth: '50px' } : {})
        }}
        onExpand={() => {
