...
const COLLAPSED_WIDTH_PX = 50;  // Ancho fijo cuando está colapsado
...
const SidePanelGroup = memo(
  ({
    ...
    // Función para convertir píxeles a porcentaje para el sidebar colapsado
    const getCollapsedSizePercentage = useCallback(() => {
      if (typeof window === 'undefined') return 3; // Server-side fallback
      const viewportWidth = window.innerWidth;
      const percentage = (COLLAPSED_WIDTH_PX / viewportWidth) * 100;
      
      console.log('[SidePanel] Collapsed size calculation:', {
        targetPx: COLLAPSED_WIDTH_PX,
        viewportWidth,
        calculatedPercentage: percentage,
        equivalentPx: (viewportWidth * percentage) / 100
      });
      
      // Limitar entre 1% y 8% para casos extremos
      return Math.max(1, Math.min(8, percentage));
    }, []);
    ...
    useEffect(() => {
      if (isSmallScreen) {
        ...
        // 🆕 Usar ancho fijo en píxeles convertido a porcentaje
        const collapsedPercentage = getCollapsedSizePercentage();
        setCollapsedSize(collapsedPercentage);
        setMinSize(defaultMinSize);
        
        console.log('[SidePanel] Effect updated collapsed size:', {
          collapsedPercentage,
          equivalentPx: (window.innerWidth * collapsedPercentage) / 100
        });
      }
    }, [isSmallScreen, defaultCollapsed, getCollapsedSizePercentage, fullPanelCollapse]);
        ...
        >
          ...
          {!hideSidePanel && interfaceConfig.sidePanel === true && (
            <SidePanel
              ...
              // 🆕 Pasar función para calcular tamaño colapsado
              getCollapsedSizePercentage={getCollapsedSizePercentage}
            />
          )}
        </ResizablePanelGroup>
        <button
